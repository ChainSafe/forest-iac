...
Snapshot = Struct.new(:network, :date, :height, :compressed, :file_name, :url_s3, :url) do
  def delete
...
def list_snapshots(chain_name = 'calibnet', bucket = 'forest-snapshots', endpoint = 'fra1.digitaloceanspaces.com')
  ...
  snapshot_format = /^([^_]+?)_snapshot_(?<network>[^_]+?)_(?<date>\d{4}-\d{2}-\d{2})_height_(?<height>\d+)\.car(?<compressed>\.zst)?(\.tmp|\.aria2)?$/
