...
# The .env file contains the chain variables that we want access to.
set -o allexport
...
# Check if a chain variable is set. If it isn't, set error=1.
check_env () {
    A="                            ";
    echo -n "${A:0:-${#1}} $1: "
    if [[ -z "${!1}" ]]; then
...
# Check that the chain variables in the .env file have been defined.
...
if [ "$error" -ne "0" ]; then
    echo "Please set the required chain variables and try again."
    exit 1
fi
